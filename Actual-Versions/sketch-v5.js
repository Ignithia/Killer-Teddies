...
function checkAttackOnP2() {
  if (P1IsAttacking && P2Alive && !P1HasHitP2 && !P2IsInvincible) {
...
function checkAttackOnP1() {
  if (P2IsAttacking && P1Alive && !P2HasHitP1 && !P1IsInvincible) {
...
//Power-Up
//Creation of powerups (the types are Speed Boost, Double Jump, Invincibility, Healing, Attack Boost)
function createPowerUps() {
  if (frameCount % 600 === 0) {
    ...
    while (!validPosition) {
      ...
      y = random(50, canvasHeight - 100);
      validPosition = true;

      //checks if the power-up would spawn too close to a player
      if (dist(P1X, P1Y, x, y) < 100 || dist(P2X, P2Y, x, y) < 100) {
        validPosition = false;
        continue;
      }

      //checks if the power-up would get stuck in a platform
      for (let platform of platforms) {
        if (
          platform.x - platform.width / 2 < x &&
          x < platform.x + platform.width / 2 &&
          platform.y - platform.height / 2 < y &&
